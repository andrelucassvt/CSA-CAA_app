...
class _EscolherInteracoesWebPageState extends State<EscolherInteracoesWebPage> {
  ...
  void cadastrarPaciente(PacienteEntity pacienteEntity) {
    final List<InteracaoEntity> interacoes = interacoesCopy
      .where((interacao) => interacao.isSelect)
      .toList();
    var listId = interacoes.map((element) => element.id!).toList();      
    controller.add(CadastrarPacienteEvent(pacienteEntity.copyWith(idInteracoes: listId)));
  }

  @override
  void initState() {
    controller.add(GetAllInteracoesEvent());
    super.initState();
  }
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: BlocConsumer<CadastroPacienteBloc, CadastroPacienteState>(
          bloc: controller,
          ...
          builder: (context,state){
            ...
            if (state is GetAllInteracoeFailure) {
              return ErrorViewWidget(
                ...
                actionButton: () async => controller.add(GetAllInteracoesEvent()),
              );
            }
          if (state is GetAllInteracoeSucess) {
